...
import { Loader2, Eye, EyeOff, Headset } from "lucide-react";
...
const SupportLogin = () => {
  ...
  if (session) return <Navigate to="/suporte" replace />;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isSignUp) {
      const { error } = await supabase.auth.signUp({
        ...
        password,
        options: {
          emailRedirectTo: window.location.origin + "/suporte",
          data: { full_name: name.trim(), role: "support" },
        },
      });
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-2xl font-bold">Suporte LAV & SEC BEACH</CardTitle>
          <CardDescription>
            {isSignUp ? "Crie sua conta de suporte" : "Acesse o painel de suporte"}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            {isSignUp && (
              <div className="space-y-2">
                ...
                <Input id="name" type="text" placeholder="Seu nome" value={name} onChange={(e) => setName(e.target.value)} required maxLength={100} />
              </div>
            )}
            <div className="space-y-2">
